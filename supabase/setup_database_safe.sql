...
DO \$\$ BEGIN CREATE TYPE public.user_role AS ENUM ('father','operator','admin'); EXCEPTION WHEN duplicate_object THEN null; END \$\$;
DO \$\$ BEGIN CREATE TYPE public.content_status AS ENUM ('IDEA','PLANNED','RESEARCHING','SCRIPT_DRAFT','REVIEW','APPROVED','RECORDING','EDITING','SCHEDULED','PUBLISHED','ANALYZING','LEARNED','BLOCKED','REJECTED','ARCHIVED'); EXCEPTION WHEN duplicate_object THEN null; END \$\$;
DO \$\$ BEGIN CREATE TYPE public.verification_status AS ENUM ('verified','unverified','disputed'); EXCEPTION WHEN duplicate_object THEN null; END \$\$;
...
DO \$\$ BEGIN CREATE TYPE public.trend_status AS ENUM ('new','approved','dismissed','converted'); EXCEPTION WHEN duplicate_object THEN null; END \$\$;
create table if not exists public.profiles(id uuid primary key references auth.users(id) on delete cascade,name text not null,role public.user_role not null default 'father',created_at timestamptz not null default now());
create table if not exists public.projects(id uuid primary key default gen_random_uuid(),name text not null,description text,status text not null default 'active',created_at timestamptz not null default now());
create table if not exists public.series(id uuid primary key default gen_random_uuid(),project_id uuid references public.projects(id) on delete set null,name text not null,description text,target_audience text,content_goal text,status text not null default 'active',created_at timestamptz not null default now());
create table if not exists public.content_items(id uuid primary key default gen_random_uuid(),series_id uuid references public.series(id) on delete set null,title text not null,content_type text not null,pillar text not null,funnel_stage text,status public.content_status not null default 'IDEA',priority int not null default 5,scheduled_date date,published_at timestamptz,sequence_order int,created_at timestamptz not null default now(),updated_at timestamptz not null default now());
create table if not exists public.content_tasks(id uuid primary key default gen_random_uuid(),content_item_id uuid not null references public.content_items(id) on delete cascade,task_type text not null,status public.task_status not null default 'todo',assigned_to uuid references public.profiles(id),due_date date,completed_at timestamptz,created_at timestamptz not null default now());
create table if not exists public.content_dependencies(content_item_id uuid not null references public.content_items(id) on delete cascade,depends_on_item_id uuid not null references public.content_items(id) on delete cascade,primary key(content_item_id,depends_on_item_id),check(content_item_id<>depends_on_item_id));
create table if not exists public.ideas(id uuid primary key default gen_random_uuid(),raw_input text not null,input_type text not null default 'text',audio_path text,transcript text,normalized_idea text,status text not null default 'new',created_by uuid references public.profiles(id),created_at timestamptz not null default now());
create table if not exists public.scripts(id uuid primary key default gen_random_uuid(),content_item_id uuid not null references public.content_items(id) on delete cascade,version int not null,content jsonb not null,status text not null default 'draft',model text,approved_by uuid references public.profiles(id),created_at timestamptz not null default now(),unique(content_item_id,version));
create table if not exists public.knowledge_documents(id uuid primary key default gen_random_uuid(),category text not null,title text not null,content text not null,source_type text not null,verification_status public.verification_status not null default 'unverified',source_url text,created_by uuid references public.profiles(id),created_at timestamptz not null default now());
create table if not exists public.knowledge_chunks(id uuid primary key default gen_random_uuid(),document_id uuid not null references public.knowledge_documents(id) on delete cascade,content text not null,embedding vector(1536),metadata jsonb not null default '{}'::jsonb,created_at timestamptz not null default now());
...
create table if not exists public.research_items(id uuid primary key default gen_random_uuid(),content_item_id uuid references public.content_items(id) on delete set null,topic text not null,findings text,status text not null default 'queued',created_at timestamptz not null default now());
create table if not exists public.sources(id uuid primary key default gen_random_uuid(),research_item_id uuid not null references public.research_items(id) on delete cascade,url text not null,title text,publisher text,published_at timestamptz,accessed_at timestamptz not null default now(),credibility numeric(4,3));
create table if not exists public.trend_items(id uuid primary key default gen_random_uuid(),topic text not null,source text not null,signal_type text,relevance_score numeric(5,2),urgency text,status public.trend_status not null default 'new',detected_at timestamptz not null default now(),metadata jsonb not null default '{}'::jsonb);
create table if not exists public.masterclasses(id uuid primary key default gen_random_uuid(),title text not null,status text not null default 'draft',created_at timestamptz not null default now());
create table if not exists public.masterclass_modules(id uuid primary key default gen_random_uuid(),masterclass_id uuid not null references public.masterclasses(id) on delete cascade,title text not null,"order" int not null,learning_objective text,status text not null default 'draft');
create table if not exists public.masterclass_sections(id uuid primary key default gen_random_uuid(),module_id uuid not null references public.masterclass_modules(id) on delete cascade,title text not null,content text not null,content_source_id uuid,status text not null default 'draft',approved_by uuid references public.profiles(id),version int not null default 1);
create table if not exists public.content_analytics(id uuid primary key default gen_random_uuid(),content_item_id uuid not null references public.content_items(id) on delete cascade,platform text not null,metric_name text not null,metric_value numeric not null,recorded_at timestamptz not null default now());
create table if not exists public.feedback(id uuid primary key default gen_random_uuid(),content_item_id uuid references public.content_items(id) on delete cascade,user_id uuid references public.profiles(id),feedback_type text not null,feedback_text text,created_at timestamptz not null default now());
create table if not exists public.agent_runs(id uuid primary key default gen_random_uuid(),agent_name text not null,input jsonb not null,output jsonb,model text,tokens int,cost numeric(12,6),status text not null,created_at timestamptz not null default now());
create table if not exists public.audit_logs(id uuid primary key default gen_random_uuid(),user_id uuid references public.profiles(id),entity_type text not null,entity_id uuid,action text not null,metadata jsonb not null default '{}'::jsonb,created_at timestamptz not null default now());
create table if not exists public.voice_examples(id uuid primary key default gen_random_uuid(),title text not null,content text not null,source_type text not null,verification_status public.verification_status not null default 'unverified',created_at timestamptz not null default now());
create table if not exists public.notification_preferences(id uuid primary key default gen_random_uuid(),user_id uuid unique not null references public.profiles(id) on delete cascade,max_daily int not null default 3,channels jsonb not null default '{"push":true}'::jsonb);
...
DO \$\$ BEGIN CREATE TYPE public.job_status AS ENUM ('queued', 'running', 'done', 'failed', 'cancelled'); EXCEPTION WHEN duplicate_object THEN null; END \$\$;
...
DO \$\$ BEGIN CREATE TYPE public.claim_verification AS ENUM ('supported', 'unsupported', 'disputed', 'needs_review'); EXCEPTION WHEN duplicate_object THEN null; END \$\$;
