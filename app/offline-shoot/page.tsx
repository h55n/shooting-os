...
export default function OfflineShootPage() {
  ...
  useEffect(() => {
    ...
    if (!id) return;
    try {
      const raw = localStorage.getItem(`shooting-script:${id}`);
  ...
  if (!snapshot) return <div className="min-h-dvh bg-black px-6 py-10 text-white"><h1 className="text-[24px] font-bold">Offline script available nahi hai</h1><p className="mt-3 text-white/65">{contentId ? 'Is script ko pehle ek baar Shooting View mein online kholna hoga.' : 'Script id nahi mila.'}</p><Link href="/content" className="mt-6 inline-flex min-h-12 items-center rounded-full bg-white px-5 font-bold text-black">Back to Content</Link></div>;
