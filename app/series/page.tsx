...
type Series = { id: string; name: string; description?: string; target_audience?: string; duration_days?: number; status: string; series_topics?: Topic[] };
...
export default function SeriesPage() {
  ...
  async function createSeries() {
    ...
    try {
      const response = await fetch('/api/series', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ name, theme, audience, durationDays }) });
      const body = await response.json();
      if (!response.ok || !body.ok) throw new Error(body.userMessage || 'Series create nahi ho saka.');
      if (!body.data.persisted) setMessage(`Demo ${durationDays}-topic plan ready; database connected hone par persist hoga.`);
      else { setName(''); setTheme(''); await load(); setSelectedSeries(body.data.id); setMessage(`${durationDays}-topic plan ready. Ab strong topics select, edit ya reorder karein.`); }
  ...
  return <div>
    <header className="mb-5"><h1 className="text-[30px] font-bold tracking-tight">Series</h1><p className="mt-1 text-[15px] text-muted-foreground">Choose 7, 15 or 30 topics first. Owner review before scripts.</p></header>
    <section className="rounded-2xl bg-card p-5 shadow-[var(--shadow-card)]">
      <h2 className="text-[17px] font-bold">Create a series</h2>
      ...
      <div className="mt-4 grid grid-cols-3 gap-2" aria-label="Series duration">{([7, 15, 30] as const).map((days) => <button key={days} onClick={() => setDurationDays(days)} className={`min-h-11 rounded-xl text-[14px] font-bold ${durationDays === days ? 'bg-foreground text-background' : 'bg-secondary'}`}>{days} days</button>)}</div>
      <button onClick={createSeries} disabled={busy !== null || name.trim().length < 3 || theme.trim().length < 3} className="mt-4 min-h-12 w-full rounded-xl bg-primary px-4 text-[15px] font-bold text-primary-foreground disabled:opacity-40">{busy === 'create' ? `Planning ${durationDays} topics…` : `Create ${durationDays}-Topic Plan`}</button>
    </section>
    ...
    {items.length > 0 && <section className="mt-6 pb-6">
      ...
      {current && <>
        <div className="mb-3 flex items-end justify-between gap-3"><div><h2 className="text-[20px] font-bold">{current.name}</h2><p className="mt-1 text-[13px] text-muted-foreground">{topics.filter((topic) => topic.status === 'scripted').length}/{current.duration_days ?? topics.length} scripted · select up to 8</p></div>{selectedTopics.length > 0 && <button onClick={buildSelected} disabled={busy !== null} className="min-h-11 rounded-full bg-primary px-4 text-[13px] font-bold text-primary-foreground">{busy === 'scripts' ? 'Building…' : `Build ${selectedTopics.length} Scripts`}</button>}</div>
