...
export default function FirstTenPage() {
  ...
  return <div>
    ...
    {!journey ? <div className="rounded-3xl bg-foreground p-6 text-background"><p className="text-[22px] font-bold">Your first ten should feel like you.</p><p className="mt-2 text-[15px] leading-6 opacity-75">System verified knowledge aur content strategy se ordered briefs banayega—ten full scripts ek saath nahi.</p><button onClick={generate} disabled={busy !== null} className="mt-5 min-h-[52px] w-full rounded-xl bg-background px-4 text-[15px] font-bold text-foreground">{busy === 'generate' ? 'Creating journey…' : 'Create My First 10 →'}</button></div> : <>
      ...
      <div className="space-y-3 pb-5">{items.map((item) => <article key={item.id} className={`rounded-2xl p-5 shadow-[var(--shadow-card)] ${item.id === active?.id ? 'bg-foreground text-background' : 'bg-card'}`}><div className="flex items-start gap-4"><div className={`grid size-9 shrink-0 place-items-center rounded-full text-[14px] font-bold ${item.id === active?.id ? 'bg-background text-foreground' : 'bg-secondary'}`}>{item.position}</div><div className="min-w-0 flex-1"><h2 className="text-[17px] font-bold leading-6">{item.title}</h2>{item.purpose && <p className={`mt-1 text-[13px] leading-5 ${item.id === active?.id ? 'opacity-70' : 'text-muted-foreground'}`}>{item.purpose}</p>}<button onClick={() => start(item)} disabled={busy !== null} className={`mt-3 min-h-11 rounded-xl px-4 text-[14px] font-bold ${item.id === active?.id ? 'bg-background text-foreground' : 'bg-primary text-primary-foreground'} disabled:opacity-40`}>{busy === item.id ? 'Creating…' : item.content_item_id ? 'Continue →' : 'Create script →'}</button></div></div></article>)}</div>
    </>}
