...
export default function IdeasPage() {
  ...
  const [tab, setTab] = useState<'owner' | 'suggested'>(searchParams.get('mode') === 'suggest' ? 'suggested' : 'owner');
  ...
  async function create(mode: 'capture' | 'suggest') {
    ...
    try {
      ...
      if (body.data.persisted === false) {
        setMessage('Demo mode: idea preview bana hai; real persistence ke liye Supabase chahiye.');
      } else {
        setText('');
        await load();
      }
  ...
  return (
    <div>
      <header className="mb-5">
        <h1 className="text-[30px] font-bold tracking-tight">Ideas</h1>
        <p className="mt-1 text-[15px] text-muted-foreground">Rough thought se strong video tak</p>
      </header>
      ...
      <div className="space-y-3 pb-4">
        {visible.length === 0 ? (
          <div className="rounded-2xl bg-card p-6 text-center text-[14px] text-muted-foreground shadow-[var(--shadow-card)]">{tab === 'owner' ? 'No ideas yet. Add one above.' : 'No suggestions yet. Ask for one strong topic.'}</div>
        ) : visible.map((idea) => {
          const proposal = proposalOf(idea);
          return (
            <article key={idea.id} className="rounded-2xl bg-card p-5 shadow-[var(--shadow-card)]">
              <div className="flex items-start justify-between gap-3">
                <div>
                  <p className="text-[12px] font-bold uppercase tracking-[0.1em] text-muted-foreground">{idea.source === 'suggested' ? 'Suggested' : 'My Idea'}</p>
                  <h2 className="mt-1 text-[18px] font-bold leading-6">{proposal.title || idea.raw_input}</h2>
                </div>
                <span className="rounded-full bg-secondary px-2.5 py-1 text-[11px] font-bold">{idea.status}</span>
              </div>
